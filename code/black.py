...
# 源目录
MyPath = 'E:\\learning\\programming\\machineLearning\\dataset\\cyclegan\\pix_black\\A\\test\\'
# 输出目录
OutPath = 'E:\\learning\\programming\\machineLearning\\dataset\\cyclegan\\pix_black\\A_burABlack\\test\\'
...
def processImage(filesoure, destsoure, name):
    ...
    rand2 = random.randint(1, 10)
    ran4 = random.randint(40, 100)
    im = cv2.imread(filesoure + name)
    im2 = operation(im)
    if rand2 > 2:
        cv2.imwrite(destsoure + name, im2, [int(cv2.IMWRITE_JPEG_QUALITY), ran4])
    else:
        cv2.imwrite(destsoure + name, im2, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
